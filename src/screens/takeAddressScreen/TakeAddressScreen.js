...
const TakeAddressScreen = (props) => {
    ...
    useEffect(() => {
        async function fetchMyAPI() {
            ...
            let url = `https://maps.googleapis.com/maps/api/place/details/json?key=AIzaSyBptxrRpSLKE2pYCk5Lqr9fg7g7rrFWPOo&placeid=${selectedSource}`;
            const response = await fetch(url, reqOpts);
            const data = await response.json();
            SetSourcelatlong(data?.result?.geometry?.location);
            console.log("data> source",data?.result?.geometry?.location.lat);
            // setiniLat(data?.result?.geometry?.location.lat);
            // setiniLong(data?.result?.geometry?.location.lng);
        }
        fetchMyAPI();
    ...
    useEffect(() => {
        async function fetchMyAPI() {
            ...
            let url = `https://maps.googleapis.com/maps/api/place/details/json?key=AIzaSyBptxrRpSLKE2pYCk5Lqr9fg7g7rrFWPOo&placeid=${selectedDest}`;
            const response = await fetch(url, reqOpts);
            const data = await response.json();
            SetDestlatlong(data?.result?.geometry?.location);
            console.log("data> source",data?.result?.geometry?.location.lat);
            // setiniLat(data?.result?.geometry?.location.lat);
            // setiniLong(data?.result?.geometry?.location.lng);
        }
    ...
    const OnGo = () => {
        ...
        if (sourcelatlong && destlatlong) {
            props.navigation.navigate(ScreensName.mainMapScreen, {
                iniLatt: parseFloat(sourcelatlong?.lat),
                iniLongg: parseFloat(sourcelatlong?.lng),
                destLatt: parseFloat(destlatlong?.lat),
                destLongg: parseFloat(destlatlong?.lng),
                sourcename: SourceAddress,
                destiname: DestAddress,
            });
        }
        setSelectedSource('');
        setSelectedDest('');
                            ...
                            <View style={{ width: '75%', height: 40 }}>
                                <GooglePlacesAutocomplete
                                    ...
                                    onPress={(data, details = null) => {
                                        ...
                                        setSelectedSource(data?.place_id);
                                        setSourceAddress(data?.description);
                                    }}
                                    query={{
                                        key: 'AIzaSyBptxrRpSLKE2pYCk5Lqr9fg7g7rrFWPOo', // Replace with your Google API key
                                        language: 'en',
                                    }}
                                    styles={{
                                        textInputContainer: {
